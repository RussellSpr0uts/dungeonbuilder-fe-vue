<template>
  <div class="wrapper" id="wrapper" @mousedown="checkType">
    <transition name="notification">
      <div v-if="showMessage" class="alert" :class="[`alert-${alertType}`, outlined ? 'alert-outline' : '']">
...
export default class Notification extends Vue {
  @Prop({type: Object, required: true}) private message: NotificationMessage;
  @Prop({type: Boolean, required: false, default: true}) private outlined: boolean;
  @Prop(Boolean) private showMessage: boolean;

  get alertType() {
    if (this.message.code === 500 || this.message.code === 503) {
      return 'danger';
    } else if (this.message.code === 401) { // Unauthorized
      return 'warning';
    }
  ...
  private checkType() {
    ...
    document.onmousemove = function checkDirection(e) {
      ...
      document.onmousemove = function elementDrag(e2: any) {
        e2 = e2 || window.event;
        pos1 = pos3 - e2.clientX;
        pos3 = e2.clientX;
        if (start - e2.clientX < -10) {
          setTimeout(() => {
            elmnt.style.left = `${offset}px`;
          }, 500);
          self.close();
        }
        if (start - e2.clientX < 0) {
